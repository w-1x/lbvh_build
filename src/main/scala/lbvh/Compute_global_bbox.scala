...
class Compute_global_bbox extends Module { // DEPTH + 1 + 1 个周期
  ...
  compute_big_bbox.io.input.bbox1 := io.input.bbox_and_tri_id.bbox
  compute_big_bbox.io.input.bbox2 := temp_bigger_bbox

  when(clock_count_reg === 0.U) {
    temp_bigger_bbox := io.input.bbox_and_tri_id.bbox
  }.otherwise {
  ...
  io.output.global_bbox_and_valid.global_bbox.maxPoint.x := mulity(
    0
  ).io.actual.out
  io.output.global_bbox_and_valid.global_bbox.maxPoint.y := mulity(
    2
  ).io.actual.out
  io.output.global_bbox_and_valid.global_bbox.maxPoint.z := mulity(
    4
  ).io.actual.out

  io.output.global_bbox_and_valid.global_bbox.minPoint.x := mulity(
    1
  ).io.actual.out
  io.output.global_bbox_and_valid.global_bbox.minPoint.y := mulity(
    3
  ).io.actual.out
  io.output.global_bbox_and_valid.global_bbox.minPoint.z := mulity(
    5
  ).io.actual.out

  when(io.input.bbox_and_tri_id.tri_id === DEPTH.U) { // temp 存在一个周期时延，乘法需要一个周期 ,tri_countReg下周期才变化
    tri_countReg := true.B
  }
  io.output.global_bbox_and_valid.valid := tri_countReg
}
