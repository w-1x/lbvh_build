...
class Compute_need_and_merge1IO extends Bundle {
...
class Compute_need_and_merge1 extends Module { // 5个时钟周期 = 2+ 1 + 1 + 1
  val io = IO(new Compute_need_and_merge1IO)
  ...
  val size = Reg(UInt(ADDR_WIDTH.W))
  ...
  }.elsewhen(clock_count_reg > 2.U && clock_count_reg <= size) {
...
// 非首node时，mergeReg(1)在clock = 4时初始化，否则在clock= 3
  ...
  when(clock_count_reg > 4.U && clock_count_reg <= 4.U + size) {
    out_valid := true.B
  }.otherwise {
    out_valid := false.B
  }
  when(clock_count_reg === 4.U + size) { // 运算完成后，还原状态
    out_merge := 0.B
    out_need_mergeReg := 0.B
    clock_count_reg := 0.B
    clockaddstartReg := false.B
  }
